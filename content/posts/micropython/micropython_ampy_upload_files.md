...
This is an update to the sixth part of a multi-part series on Micropython. I had trouble in class today uploading a .py-file to an ESP8266 microcontroller running MicroPython using a command line tool called **ampy**. There was a trick getting **ampy** to work on with the ESP8266 microcontroller that I want to share in case other people come accross the same problem. In this post, I'll show you how I uploaded upload **_.py_** files to an ESP8266 microcontroller using Python and a Python package called **ampy**.
...
Before you upload files on the microcontroller, it is a good idea to see what files are already stored on the microcontroller. Seeing which files are on the board is also a good test to make sure **ampy** works properly.
...
You can modify **_boot.py_** on your computer and then upload the modified version of **_boot.py_** to the ESP8266. The ESP8266 will then run whatever is in **_boot.py_** the next time the microcontroller restarts.
...
Now you can see if the changes you made work, and the ```esp.osdebug(None)``` command really worked.
...
You can also upload additional .py-files to the board with the same **ampy** ```put``` command. A common pattern is to have very minimal code in **_main.py_**.  You can use **_main.py_** to import functions from other .py-files stored on the board and run them.
